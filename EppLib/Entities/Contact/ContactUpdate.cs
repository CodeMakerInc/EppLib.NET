...
namespace EppLib.Entities
{
    public class ContactUpdate : ContactBase<ContactUpdateResponse>
    {
        ...
        protected override XmlElement BuildCommandElement(XmlDocument doc, XmlElement commandRootElement)
        {
            ...
            var add_element = getAddRemoveElement(doc, ToAdd, "contact:add");
            ...
            var remove_element = getAddRemoveElement(doc, ToRemove, "contact:rem");
            ...
            // RFC 5733 puts authInfo inside chg; the Password property would otherwise land after it.
            var authInfo = ContactChange?.AuthInfo ?? (string.IsNullOrWhiteSpace(Password) ? null : Password);

            if (ContactChange != null || authInfo != null)
            {
                ...
                // RFC 5733 order: postalInfo, voice, fax, email, authInfo, disclose.
                ...
                if (authInfo != null)
                {
                    var authInfoElement = AddXmlElement(doc, change_element, "contact:authInfo", null, namespaceUri);
                    AddXmlElement(doc, authInfoElement, "contact:pw", authInfo, namespaceUri);
                }
        ...
        private XmlElement getAddRemoveElement(XmlDocument doc, EppContactUpdateAddRemove addRemoveItems, string tagName)
        {
            ...
            if (addRemoveItems != null)
            {
                     ...
                     addRemoveItems.Status.Count > 0)
                {
                    ...
                    foreach (var status in addRemoveItems.Status)
                    {
                        ...
                        var status_element = AddXmlElement(doc, add_remove_element, "contact:status", status.Value, namespaceUri);
